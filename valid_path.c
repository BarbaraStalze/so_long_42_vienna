...
/*   Updated: 2026/03/23 17:42:00 by bastalze         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "so_long.h"
...
static char	**ft_copy_map(t_game *game)
{
	...
	while (game->map[i])
	{
		...
		if (!game->test_map[i])
		{
			...
			ft_error("Malloc failed in copy map", game);
		}
		while (game->map[i][j])
		{
...
void	ft_valid_path(t_game *game)
{
	...
	if (!game->test_map)
		ft_error("Malloc failed in ft_valid_path", game);
	...
	if (game->exit_found == 0 || game->collectibles_found != game->collectibles)
	{
		ft_free_array(game->test_map);
		errno = EINVAL;
		ft_error("Map has no valid path", game);
	}
	ft_free_array(game->test_map);
}
